...
export class FeedbackComponent implements OnInit {
  ...
  calculateKpi(): void {
    ...
    // HAUSHALT
    let sumHaushalt = 0;
    const hh: FeedbackDataModel[] = this.allFeedbacks.filter(f => f.productCategory === "HAUSHALT");
    hh.forEach(f => {
    ...
    // LEBEN
    let sumLeben = 0;
    const l: FeedbackDataModel[] = this.allFeedbacks.filter(f => f.productCategory === "LEBEN")
    l.forEach(f => {
      sumLeben += f.rating
    })
    ...
    const hp: FeedbackDataModel[] = this.allFeedbacks.filter(f => f.productCategory === "HAFTPFLICHT")
    hp.forEach(f => {
    ...
    // REISEN
    let sumReisen = 0;
    const r: FeedbackDataModel[] = this.allFeedbacks.filter(f => f.productCategory === "REISE")
    r.forEach(f => {
      sumReisen += f.rating
    })
    ...
    // FAHRZEUG
    let sumFahrzeug = 0;
    const f: FeedbackDataModel[] = this.allFeedbacks.filter(f => f.productCategory === "FAHRZEUG")
    f.forEach(f => {
    ...
    // KRANKEN
    let sumKranken = 0;
    const k: FeedbackDataModel[] = this.allFeedbacks.filter(f => f.productCategory === "KRANKEN")
    k.forEach(f => {
      sumKranken += f.rating
    })
